...
const loadListMusic = async () => {
    try {
        ...
        const cmd = 'exiftool -artist -title -duration -picture -album -b -j ' + listUrls.join(' ');
...
const saveUrlRecent = async (event, url) => {
    try {
        ...
        if (!data.toString().includes(url)) {
            ...
            await deleteMusicRecent(null, url);
            await appendUrlIntoRecentMusic(url);
        }
    } catch (error) {
        console.log(error);
...
const deleteMusicPlaylist = async (event, name, url) => {
    try {
        let data = await fs.readFileSync('/home/noir/Desktop/PBL4-4/MusicApp/src/API/' + name + '.txt');
        data = data.toString();
        data = data.replace(url + '\n', '');
        await fs.writeFileSync('/home/noir/Desktop/PBL4-4/MusicApp/src/API/' + name + '.txt', data);
    } catch (error) {
        console.log(error);
    }
};
...
const saveMusicIntoPlayList = async (event, name, url) => {
    try {
        ...
        if (!data.toString().includes(url)) {
            console.log(
                'fileMP3Handle: inside mehtod saveMusicIntoPlayList(), message: thuc hien them url vao file(da tao)',
            );
...
const saveNamePlayList = async (name) => {
    try {
        ...
        if (!data.toString().includes(name + '\n')) {
            console.log(
                'fileMP3Handle: inside mehtod saveNamePlayList(), message: thuc hien them url vao file(da tao)',
            );
